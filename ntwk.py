...
class LIFNtwkI(object):
    ...
    def __init__(self, c_m, g_l, e_l, v_th, v_r, t_r, w_r, w_u, i_b, f_b, t_b, t_a, noise_var=0, sparse=True):
        # ntwk size
        n = w_r.shape[0]
    ...
    def run(self, dt, clamp, i_ext, spks_u=None, sigma_b=0):
        ...
        for t_ctr in range(len(i_ext) - l_b):
            ...
            if t_ctr in clamp.v:  # check for clamped voltages
                ...
                # update v
                vs[t_ctr, :] = v + (dt/c_m)*i_total + np.random.normal(size=v.shape, scale=np.sqrt(self.noise_var))
